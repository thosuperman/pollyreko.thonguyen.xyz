...
AWS.config.credentials = new AWS.CognitoIdentityCredentials({
  IdentityPoolId: "ap-northeast-1:57fb7054-e542-4877-a518-947426679100"
});
...
function generateSpeech() {
  ...
  var text =
    "Hi, my name is Polly. In this picture I see " +
    faces.FaceDetails.length +
    " face" +
    (faces.FaceDetails.length != 1 ? "s" : "") +
    ". ";
  ...
  for (var i = 0; i < faces.FaceDetails.length; i++) {
    ...
    if (face["Emotions"])
      text +=
        sortByProperty(face.Emotions, "Confidence")
          .reverse()[0]
          .Type.toLowerCase() + " ";
  ...
  for (var i = 0; i < labels.length; i++) {
    log(
      "   " + labels[i].Name + " (" + Math.round(labels[i].Confidence) + "%)\n"
    );
  ...
  log(
    'Synthesizing speech for 4 first objects with Polly:\n    "' + text + '"\n'
  );
  ...
  rekognition.detectFaces(params, function(err, data) {
    ...
    // an error occurred
      ...
      for (var i = 0; i < data.FaceDetails.length; i++) {
        ...
        ["Gender", "Smile", "Eyeglasses", "Beard", "Mustache"].forEach(function(
          attr
        ) {
          log(
            "    " +
              ...
              face[attr].Value +
              " (" +
              Math.round(face[attr].Confidence) +
              "%)\n"
          );
        });
        ...
        context2d.fillText(
          i + 1,
          box.Left * canvas.width + 2,
          box.Top * canvas.height + fontSize
        );
        context2d.restore();
      }
